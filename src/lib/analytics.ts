...
import type { BsTokenTransfer, BsTx, ChainFetchResult } from "./blockscout";
import type {
  ActivityCategory,
  AssetStat,
  BehaviorSignals,
...
export function normalizeHoldings(cfg: ChainConfig, items: ChainFetchResult["holdings"]): HoldingStat[] {
  const out: HoldingStat[] = [];
  for (const it of items) {
    const decimals = it.token?.decimals ? Number(it.token.decimals) : null;
    const type = it.token?.type ?? "unknown";
    let amount: number | null = null;
    if (it.value && decimals !== null && Number.isFinite(decimals) && type === "ERC-20") {
      amount = Number(it.value) / 10 ** decimals;
    }
    const rate = it.token?.exchange_rate ? Number(it.token.exchange_rate) : null;
    out.push({
      chain: cfg.id,
      ...
      type,
      amount,
      usdValue: amount !== null && rate !== null && Number.isFinite(rate) ? amount * rate : null,
...
export function computeAnalytics(input: AnalyticsInput): WalletAnalytics {
  const { activity, transfers, holdings } = input;
  ...
  const signalDetails: WalletAnalytics["signalDetails"] = {
    ...
    dormancy: `longest quiet stretch ${longestQuietDays.toFixed(0)}d across ${ageDays.toFixed(0)}d wallet age`,
