...
export async function fetchChainData(
  ...
  // --- current holdings ---
  const totalCount = Number(result.counters?.transactions_count ?? 0);
  if (totalCount > 0 || result.txs.length > 0) {
    progress.step = "holdings";
    try {
      let params: Record<string, string | number> | null = null;
      for (let page = 0; page < MAX_HOLDING_PAGES; page++) {
        const url: string = `${base}/addresses/${address}/tokens?type=ERC-20%2CERC-721%2CERC-1155${pageParams(params)}`;
        const data: BsListResponse<BsHolding> = await fetchJson<BsListResponse<BsHolding>>(url, signal);
        result.holdings.push(...(data.items ?? []));
        if (!data.next_page_params) break;
        params = data.next_page_params;
        await sleep(120);
      }
