...
import { ScanEye, ShieldAlert, ArrowUpRight, Wallet, Loader2 } from "lucide-react";
import { AuthButton } from "./AuthButton";
import type { GenLayerWalletState } from "@/hooks/useGenLayerWallet";

function GenLayerWalletButton({ wallet }: { wallet: GenLayerWalletState }) {
  if (wallet.checking) {
    return (
      <span className="flex h-9 items-center gap-2 rounded-md border border-line px-3.5 font-mono text-[11px] text-dim">
        <Loader2 className="h-3.5 w-3.5 animate-spin" />
      </span>
    );
  }
  if (!wallet.prereq.hasMetaMask) return null;

  if (wallet.address) {
    return (
      <span className="flex h-9 items-center gap-2 rounded-md border border-mint/40 bg-mint/10 px-3.5 font-mono text-[11px] text-mint">
  ...
  return (
    ...
    >
      ...
      <span className="hidden sm:inline">GenLayer wallet</span>
    </button>
  );
}
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 md:px-8">
        ...
        <div className="flex items-center gap-3">
          <span className="mono-label hidden items-center gap-2 rounded-full border border-mint/35 bg-mint/10 px-3 py-1.5 text-mint lg:flex">
            ...
            GenLayer consensus · live
          </span>
          {wallet ? <GenLayerWalletButton wallet={wallet} /> : null}
          <AuthButton />
          {right}
          <a
        ...
        <div className="grid gap-10 md:grid-cols-[1.3fr_1fr_1fr]">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm text-mute">
              {[
                "A complete, inspectable evidence report",
                "Links back to the original public records",
                "Independent verification by GenLayer validators",
                "A lasting on-chain record of the verdict",
              ].map((t) => (
          ...
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-mute">
              Six EVM networks, one public record, and a verdict that can be checked against the
              evidence behind it. No wallet connection, ever.
            </p>
