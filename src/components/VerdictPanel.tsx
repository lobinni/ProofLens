...
import { Gauge, Loader2, ListChecks, FileText, ShieldQuestion, ExternalLink, RotateCcw } from "lucide-react";
import type { AttestationInfo, AttestedReport, VerdictClass } from "@/lib/types";
import { contractExplorerUrl } from "@/lib/genlayer";
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-ink-2/60 p-7 md:p-9">
      <div className="flex items-center gap-3">
        ...
        <p className="mono-label text-amber">Consensus verdict pending</p>
      </div>
      <h2 className="mt-5 max-w-2xl text-4xl font-semibold tracking-[-0.02em] text-bone md:text-5xl">
        Evidence sealed.
        <span className="font-serif-i ml-3 font-normal text-mute">Consensus is warming up.</span>
      </h2>
      <p className="mt-5 max-w-xl text-sm leading-relaxed text-mute">
        The full evidence report below is complete and sealed with its SHA-256 commitment. The
        on-chain verdict could not be requested yet because the attestation service is
        temporarily unavailable — retry in a moment and the sealed evidence is reused exactly as
        committed, nothing is recomputed.
      </p>

      {attestError && (
        <div className="mt-5 rounded-lg border border-amber/30 bg-amber/[0.05] p-4">
          <p className="font-mono text-[11px] leading-relaxed break-all text-amber">{attestError}</p>
        </div>
      )}

      <div className="mt-7 flex flex-wrap items-center gap-4">
        <button
          onClick={() => void onRetryAttest()}
          disabled={retryingAttest}
        ...
        >
          ...
          {retryingAttest ? "Requesting…" : "Request consensus verdict"}
        </button>
        <p className="max-w-sm font-mono text-[10px] leading-relaxed text-dim">
          Cadence, signals, graph, ledger and evidence below are final — only the on-chain verdict
          is still missing.
        </p>
      </div>
    </div>
...
function PendingVerdict({ attestation }: { attestation: AttestationInfo | null }) {
  return (
    <div className="grid gap-px overflow-hidden rounded-xl border border-line bg-line">
      <div className="bg-ink-2/60 p-7 md:p-9">
        ...
        <p className="mt-5 max-w-lg text-sm leading-relaxed text-mute">
          ...
          This usually takes a few minutes — this page keeps polling and updates itself.
        </p>
        {attestation && (
          <div className="mt-6 space-y-2 font-mono text-[11px] text-mute">
            <p>
              tx · <span className="text-amber">{attestation.transactionHash}</span>
            </p>
            <a
...
export function VerdictPanel({
  ...
  if (!consensus) return <PendingVerdict attestation={attestation} />;
